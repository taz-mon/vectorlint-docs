...
VectorLint sends your content to an LLM for evaluation. Before running any checks, you need to tell VectorLint which provider to use and supply credentials for it.
...
This means you can set a global default provider in `config.toml` and override it per project with a `.env` file, or override both with environment variables in CI/CD.
...
VectorLint accesses Claude models through Amazon Bedrock. AWS credentials are optional if you have an IAM role attached to your environment or a credential profile configured in `~/.aws/credentials`.
...
<Note>
If your environment already provides AWS credentials through an IAM role or `~/.aws/credentials`, you can omit `AWS_ACCESS_KEY_ID` and `AWS_SECRET_ACCESS_KEY` from your VectorLint configuration.
</Note>

## Search provider (optional)

The `technical-accuracy` evaluator verifies factual claims against live web search. This requires a separate search provider credential. VectorLint currently supports [Perplexity](https://www.perplexity.ai/) for this purpose.
...
You only need a search provider if your rule packs use the `technical-accuracy` evaluator. Standard `base` evaluator rules do not require it.
...
**Use separate keys per project or team.** Project-scoped keys are easier to rotate if compromised and give you granular cost tracking.
...
**`Rate limit exceeded`** — Reduce the `Concurrency` setting in `.vectorlint.ini`, upgrade your API tier with the provider, or switch to a provider with higher rate limits.
